...
CI will hydrate `clients.toml` from a Base64 repository secret named `CLIENTS_TOML_ENCODED` and run the batch audits. To create the secret locally (Linux/macOS):
...
gh secret set CLIENTS_TOML_ENCODED --body "$(cat clients.toml.b64)" --repo brademus/ADA-Lab
```
...
## Notes
- `clients.toml` is intentionally ignored in `.gitignore` for local usage — CI must supply it via `CLIENTS_TOML_ENCODED`.
